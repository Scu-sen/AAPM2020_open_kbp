...
#             p = next(iter(dropout))
            ...
            self.stages.append(nn.Sequential(*block)) 
        block = [SeparableConv2d(features_out, 1, kernel_size=kernel_size,stride=1, padding=padding+convdilation-1, dilation=convdilation), nn.Sigmoid()]
        self.stages.append(nn.Sequential(*block)) 
...
class ConvolutionalDecoder(nn.Module):
    ...
    """
    def __init__(self,n_features_output,num_hidden_features,kernel_size,padding,n_resblocks,dropout_min=0,dropout_max=0.2,blockObject=ResidualBlock,batchNormObject=nn.BatchNorm2d,n_latent=100):
        """
...
#         self.skipMergers = nn.ModuleList()
        ...
        num_hidden_features = np.array([n_latent] + list(num_hidden_features)[:-1])
