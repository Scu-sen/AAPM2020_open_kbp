...
#         set_trace()
        ...
        x = F.relu(self.upbn1(self.upconv1(x5)))  # [1, 256, 16, 16, 16]
        x = torch.cat([x, x4], dim=1)  # [1, 512, 16, 16, 16]
        x = F.relu(self.upbn2(self.upconv2(x)))  # [1, 128, 16, 16, 16]
        x = torch.cat([x, x3], dim=1)  # [1, 256, 16, 16, 16]
        x = F.relu(self.upbn3(self.upconv3(x)))  # [1, 64, 32, 32, 32]
        x = torch.cat([x, x2], dim=1)  # [1, 128, 32, 32, 32]
        x = F.relu(self.upbn4(self.upconv4(x)))  # [1, 64, 64, 64, 64]
        x = torch.cat([x, x1], dim=1)  # [1, 128, 64, 64, 64]
        x = F.relu(self.upbn5(self.upconv5(x)))  # [1, 32, 128, 128, 128]
