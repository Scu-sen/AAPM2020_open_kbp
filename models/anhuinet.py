...
class DilatedConvolutions(nn.Module):
    ...
    """
    def __init__(self, n_channels, n_convolutions, dropout, dilations=None):
...
class UNet(nn.Module):
    ...
    """
    def __init__(self, in_channels, out_channels, num_hidden_features,n_resblocks,num_dilated_convs, dropout_min=0, dropout_max=0, gated=False, padding=1, kernel_size=3,group_norm=32,convdilation=1,augautoenc=None,dil_dilations=None):
        """
        ...
        if num_dilated_convs > 0:
            ...
            self.dilatedConvs = DilatedConvolutions(num_hidden_features[-1],num_dilated_convs,dropout_max, dilations=dil_dilations) # <v11 uses dilatedConvs2
        else:
