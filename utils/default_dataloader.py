import os
import numpy as np
from utils.general_functions import get_paths, load_file
...
class DefaultDataLoader:
    ...
    def __init__(self, file_paths_list, batch_size=2, patient_shape=(128, 128, 128), shuffle=True,
                 mode_name='training_model', pseudo_path=None):
    ...
    def load_and_shape_data(self, path_to_load):
        ...
        if '.csv' in path_to_load:
            ...
            pseudo = True if '_pseudo' in path_to_load else False
            path_to_load = path_to_load.replace('_pseudo', '')
            files_to_load = get_paths(path_to_load, ext='')
            # Load files and get names without file extension or directory
            if pseudo:
                files_to_load = [f for f in files_to_load if not 'dose.csv' in f]
                files_to_load.append(os.path.join(self.pseudo_path, files_to_load[0].split('/')[-2], 'dose.csv'))
